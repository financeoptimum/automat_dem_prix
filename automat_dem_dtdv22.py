...
def envoyer_mail_DTD(nom_prospect, volume, code_postal, ville_depart, ville_destination, pays_destination, date_echeance, poe_destination, adresse_mail_destinateur, mdp_adresse_mail_destinateur, noms_transitaires, adresses_mail_destinataires_transitaires, noms_agents_livraison, adresses_mail_destinataires_agents_livraison, agents_livraison_groupage, noms_agents_etrangers, adresses_mail_rates_request_ds, notes_Dem_Emb_Export, notes_Dem_Export_Mer, notes_rates_request_ds, notes_Dem_Emb_Os_Retour_GM):
    try:
        ...
        if float(volume) < 15 and verifier_pays(pays_destination) :
            type_demande = 1
...
''')
    ...
    destinataires_dem_groupage = "\n".join([f" {i+1}. {email}" for i, email in enumerate(agents_livraison_groupage)])
...
<html>
  <body>
    ...
    <ul>
      <li><b>Container : 20ft OU 40ft </b></li>
...
'''
                    ...
                    message['Subject'] = f''' Ref. {nom_prospect} / Demande Fret Export 20ft ou 40ft : de {code_postal} {ville_depart}'''
...
<html>
  <body>
    ...
    <p>Best regards,</p>
    <p><b>Rates Department</b></p>
    <p>OPTIMUM MOBILITY Global</b><br>Mobility Solutions</p>
    <p>Phone: (+33) 09 67 24 16 14</p>
    <p>Email: <a href="mailto:rates@optimum-mobility.com">rates@optimum-mobility.com</a></p>
    </div>
  </body>
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.notes_Dem_Emb_Export = ttk.Entry(self.frame, width=50)
...
#         self.notes_Dem_Emb_Export.grid(row=9, column=1, padx=10, pady=5)
        ...
        self.notes_Dem_Export_Mer = ttk.Entry(self.frame, width=50)
...
#         self.notes_Dem_Emb_Export.grid(row=9, column=1, padx=10, pady=5)
        ...
        self.notes_Rates_Request_Destination_Service = ttk.Entry(self.frame, width=50)
    ...
    def send_mail(self):
        ...
        notes_Dem_Emb_Export = self.notes_Dem_Emb_Export.get()
        notes_Dem_Export_Mer = self.notes_Dem_Export_Mer.get()
        notes_rates_request_ds = self.notes_Rates_Request_Destination_Service.get()
        notes_Dem_Emb_Os_Retour_GM = ''
        
        print("Les agents de livraison avant lecture du champ : \n", adresses_mail_destinataires_agents_livraison   )
        ...
        print("Les adresses transitaires avant lecture du champ : \n", adresses_mail_destinataires_transitaires)
        ...
        print("Les adresses mails agent etranger DS : \n", adresses_mail_rates_request_ds)

        # Enlever les cases '' des listes des noms et mails des agents
        adresses_mail_destinataires_agents_livraison = [element for element in adresses_mail_destinataires_agents_livraison if element]
        adresses_mail_destinataires_transitaires = [element for element in adresses_mail_destinataires_transitaires if element]
        adresses_mail_rates_request_ds = [element for element in adresses_mail_rates_request_ds if element]

        noms_agents_livraison = [element for element in noms_agents_livraison if element]
        noms_transitaires = [element for element in noms_transitaires if element]
        noms_agents_etrangers = [element for element in noms_agents_etrangers if element]
...
#adresses_mail_destinataires_transitaires = ["mc.grandjean@herport.fr","brouard.g@leonvincent.fr"]
...
noms_transitaires = ["financetransitaire", "ratestransitaire"]
